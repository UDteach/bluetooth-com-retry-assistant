...
- PIN や専用コードが不要な機器では、通常は Windows の確認ダイアログを承認するだけで進みます。
- ペアリング時は Windows の確認ダイアログや機器側の操作が必要になることがあります。Windows がユーザー承認を要求した場合、完全な無人ペアリングは拒否されることがあります。
...
Windows が `ユーザーが認証されていない` と返す場合は、アプリ本体または Windows 設定画面でペアリング確認を承認しながら `scripts/watch_com_delta.ps1` で COM の増加を確認してください。
